<template>
  <div class="vux-x-input weui-cell"
    ...
    <div class="weui-cell__ft">
      ...
      <icon type="warn"
        class="vux-input-icon"
        v-show="novalidate && iconType === 'error'"></icon>

      <slot name="right"></slot>
      <div v-if="hasRightFullHeightSlot"
        class="vux-x-input-right-full">
...
export default {
  ...
  created() {
    ...
    if (
      ...
      console.log('初始化时校验值是否为空，如果为空则valid为false')
  ...
  methods: {
    ...
    reset(value = '') {
      ...
      this.firstError = ''
      console.log('reset重置了，并且valid变成true')
      this.valid = true
    },
    clear() {
      this.currentValue = ''
    ...
    onBlur($event) {
      this.setTouched()
      console.log('失去焦点了，即将触发validate校验')
      this.validate()
      this.isFocus = false
    ...
    getError() {
      console.log(`获取错误名列表:${Object.keys(this.errors)}`, 'getError中')
    ...
    validate() {
      ...
      if (!this.currentValue && this.required) {
        console.log(
          '如果当前值为空，并且为必填valid设置为fasle this.errors.required设置为必填哦 即将触发getError函数'
        )
      ...
      if (typeof this.isType === 'function') {
        ...
        */
        ...
        console.log(
          `istype校验方式为函数，判断将校验结果${validStatus.valid}传给valid`
        )
        if (!this.valid) {
          console.log(
            `如果校验结果为false，触发getError更新firstError提示语this.errors.format =${
              validStatus.msg
            }赋值给 this.errors.format`
          )
          ...
          console.log(
            `如果校验结果为true，删除 this.errors.format =${validStatus.msg}`
          )
          delete this.errors.format
        }
      }
      ...
      console.log('如果都符合条件valid设置为true')
      this.valid = true
    },
    validateEqual() {
  ...
  watch: {
    ...
    valid() {
      console.log('valid 变化触发，即将触发getError函数')

      this.getError()
    },
    value(val) {
      console.log(`value变化成${val}`)
    ...
    currentValue(newVal, oldVal) {
      console.log(
        `newval:${newVal}equalWidth:${this.equalWidth}当前值发生变化了`
      )
      // 如果不校验是否相同并且有新值
      ...
      if (newVal && this.equalWith) {
        ...
        console.log(
          `未设置选项是否相同，并且currentValue变成${newVal}，进行validate校验`
        )
